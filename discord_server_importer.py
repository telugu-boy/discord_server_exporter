...
async def append_emojis(
    ...
    for emoji in emojis:
        ...
        logging.info(f"Downloaded and appending emoji '{emoji['name']}' ({file_size}b) for server '{existing_guild.name}'")
...
async def append_roles(
    ...
    sorted_server_roles = list(reversed(sorted(roles, key=lambda role: int(role["position"]))))[:-1]
